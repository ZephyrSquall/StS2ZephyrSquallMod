...
{
    [HarmonyPostfix]
    private static void CallOverflowHookPatch(ref Task<IEnumerable<CardModel>> __result,
    ...
    private static async Task<IEnumerable<CardModel>> AsyncWrapper(Task<IEnumerable<CardModel>> originalTask,
        PlayerChoiceContext choiceContext, decimal count, Player player, bool fromHandDraw)
    {
        var result = await originalTask;
        await DecrementLaserFocus(player);
        var timesOverflowed = count - result.Count();
        for (var i = 0; i < timesOverflowed; i++) await ZephyrHooks.OnOverflow(choiceContext, player, fromHandDraw);
        return result;
    }
    ...
    private static async Task DecrementLaserFocus(Player player)
    {
        ...
        if (laserFocusPower != null) await laserFocusPower.DecrementUnhandledSkips();
    }
}