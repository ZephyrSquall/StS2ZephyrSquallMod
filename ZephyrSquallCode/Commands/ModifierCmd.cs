...
public class ModifierCmd
{
    ...
    public static async Task AddDeftToUpTo(PlayerChoiceContext choiceContext, Player player, int deftAmount, int maxCardAmount,
        AbstractModel source, PileType pileType = PileType.Hand)
    {
        LocString prompt = new("card_selection", "TO_ADD_DEFT_TO_UP_TO");
        prompt.Add("Deft", deftAmount);
        CardSelectorPrefs prefs = new CardSelectorPrefs(prompt, 0, maxCardAmount);
        await AddDeftWithPrefs(choiceContext, player, deftAmount, prefs, source, pileType);
    }
        ...
        AbstractModel source, PileType pileType = PileType.Hand)
    {
        ...
        prompt.Add("Deft", deftAmount);
        CardSelectorPrefs prefs = new CardSelectorPrefs(prompt, 0, int.MaxValue);
        await AddDeftWithPrefs(choiceContext, player, deftAmount, prefs, source, pileType);
    }

    private static async Task AddDeftWithPrefs(PlayerChoiceContext choiceContext,
        Player player,
        int deftAmount,
    ...
    {
        IEnumerable<CardModel> selectedCards = pileType == PileType.Hand
            ? await CardSelectCmd.FromHand(choiceContext, player, prefs, (Func<CardModel, bool>)(c => c.GainsBlock), source)
            : await CardSelectCmd.FromSimpleGrid(choiceContext, pileType.GetPile(player).Cards.Where(c => c.GainsBlock).OrderBy(c => c.Rarity).ThenBy(c => c.Id).ToList(), player, prefs);
    ...
    public static async Task AddHonedToUpTo(PlayerChoiceContext choiceContext, Player player, int honedAmount, int maxCardAmount,
        AbstractModel source, PileType pileType = PileType.Hand)
    {
    ...
    {
        IEnumerable<CardModel> selectedCards = pileType == PileType.Hand
            ? await CardSelectCmd.FromHand(choiceContext, player, prefs, (Func<CardModel, bool>)(c => c.Type == CardType.Attack), source)
            : await CardSelectCmd.FromSimpleGrid(choiceContext, pileType.GetPile(player).Cards.Where(c => c.Type == CardType.Attack).OrderBy(c => c.Rarity).ThenBy(c => c.Id).ToList(), player, prefs);
