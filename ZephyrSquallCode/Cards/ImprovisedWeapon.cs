...
public class ImprovisedWeapon() : ZephyrSquallCard(1, CardType.Attack, CardRarity.Rare, TargetType.AnyEnemy)
{
    ...
    [
        new DamageVar(CurrentDamage, ValueProp.Move), new("Decrease", 2)
    ];
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay play)
    {
        ...
        if (!(DeckVersion is ImprovisedWeapon deckVersion)) return;
        deckVersion.DebuffFromPlay(lostDamage);
    }

    protected override void OnUpgrade() => DynamicVars["Decrease"].UpgradeValueBy(-1);
    ...
    {
        CurrentDamage = Math.Max(30 - DecreasedDamage, 0);
    }
}